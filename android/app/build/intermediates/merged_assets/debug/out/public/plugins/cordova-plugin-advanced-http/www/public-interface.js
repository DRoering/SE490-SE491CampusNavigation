cordova.define("cordova-plugin-advanced-http.public-interface", function (
  require,
  exports,
  module
) {
  module.exports = function init(
    ...
    function getBasicAuthHeader(username, password) {
      return {
        Authorization:
          "Basic " + helpers.b64EncodeUnicode(username + ":" + password),
      };
    }
    ...
    function setServerTrustMode(mode, success, failure) {
      ...
      return exec(success, failure, "CordovaHttpPlugin", "setServerTrustMode", [
        helpers.checkSSLCertMode(mode),
      ]);
    }
    ...
    function setClientAuthMode() {
      ...
      return exec(success, failure, "CordovaHttpPlugin", "setClientAuthMode", [
        mode,
        options.alias,
        options.rawPkcs,
        options.pkcsPassword,
      ]);
    }
    ...
    function sendRequest(url, options, success, failure) {
      ...
      url = urlUtil.appendQueryParamsString(
        url,
        urlUtil.serializeQueryParams(options.params, true)
      );

      var headers = helpers.getMergedHeaders(
        url,
        options.headers,
      ...
      var onSuccess = helpers.injectCookieHandler(
        url,
        helpers.injectRawResponseHandler(options.responseType, success, failure)
      );
      ...
      switch (options.method) {
        ...
        case "put":
        case "patch":
          helpers.processData(options.data, options.serializer, function (
            data
          ) {
            exec(onSuccess, onFail, "CordovaHttpPlugin", options.method, [
              url,
              data,
              options.serializer,
              headers,
          ...
          break;
        case "upload":
          var fileOptions = helpers.checkUploadFileOptions(
            options.filePath,
            options.name
          );
          exec(onSuccess, onFail, "CordovaHttpPlugin", "uploadFiles", [
            url,
            headers,
            fileOptions.filePaths,
            fileOptions.names,
            options.timeout,
            options.followRedirect,
            options.responseType,
        ...
        case "download":
          ...
          var onDownloadSuccess = helpers.injectCookieHandler(
            url,
            helpers.injectFileEntryHandler(success)
          );
          exec(onDownloadSuccess, onFail, "CordovaHttpPlugin", "downloadFile", [
            url,
            headers,
            filePath,
            options.timeout,
    ...
    function put(url, data, headers, success, failure) {
      return publicInterface.sendRequest(
        url,
        { method: "put", data: data, headers: headers },
        success,
        failure
      );
    }
    ...
    function options(url, params, headers, success, failure) {
      return publicInterface.sendRequest(
        url,
        { method: "options", params: params, headers: headers },
        success,
        failure
      );
    }
