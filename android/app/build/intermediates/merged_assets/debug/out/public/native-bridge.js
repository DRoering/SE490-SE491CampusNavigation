...
(function (win) {
  ...
  capacitor.DEBUG =
    typeof capacitor.DEBUG === "undefined" ? true : capacitor.DEBUG;
  ...
  capacitor.convertFileSrc = function convertFileSrc(url) {
    ...
    if (url.startsWith("file://")) {
      return (
        window.WEBVIEW_SERVER_URL + url.replace("file://", "/_capacitor_file_")
      );
    }
    if (url.startsWith("content://")) {
      return (
        window.WEBVIEW_SERVER_URL +
        url.replace("content:/", "/_capacitor_content_")
      );
    }
    return url;
  };
   ...
   */
  capacitor.toNative = function toNative(
    pluginId,
    methodName,
    options,
    storedCallback
    ...
    try {
      if (capacitor.isNative) {
        ...
        orgConsole.warn.call(
          win.console,
          "browser implementation unavailable for: " + pluginId
        );
      }
    } catch (e) {
  ...
  capacitor.nativeCallback = function (
    pluginId,
    methodName,
    options,
    callback
  ...
  capacitor.addListener = function (pluginId, eventName, callback) {
    var callbackId = capacitor.nativeCallback(
      pluginId,
      "addListener",
      {
        eventName: eventName,
  ...
  capacitor.removeListener = function (
    pluginId,
    callbackId,
    eventName,
    callback
